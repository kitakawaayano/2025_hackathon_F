...
export const SignUp: React.FC = () => {
    ...
    const onSubmit = async ( data: any ) => {
        try {
            const uniqueCheck = await axios.get(`https://two025-hackathon-json.onrender.com/users?user_name=${data.user_name}`);
            if (uniqueCheck.data.length > 0) {
                toast.error("アカウントの作成に失敗しました");
                return;
            }

            const response = await axios.post('https://two025-hackathon-json.onrender.com/users', {
